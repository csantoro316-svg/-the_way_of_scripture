# Content Sources

This folder stores source material used to build the site.

## Structure

- `studies/`: study drafts, outlines, and source copy.
- `passages/`: passage-focused source material.
- `assets/`: images, PDFs, and supporting files.

## Working Rule

- You do not need to fully pre-structure site content before dropping files here.
- Put study material in `studies/` and passage-specific material in `passages/`.
- Codex can normalize these source files into site-ready structure during implementation.

## Recommended Format

- Prefer Markdown files for authored source material.
- Word documents and PDFs can live in `assets/` when needed as reference material.
